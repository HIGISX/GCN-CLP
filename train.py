...
class Train:
    ...
    def validate(self, validate_graphs, model, problem="mclp"):
        ...
        for gidx, graph in enumerate(tqdm(validate_graphs)):
            ...
            if problem == "mclp":
                y_pre_covering, y_covering = mclp_greedy(prob_maps, num_prob_maps, A, labels.tolist())
        ...
        logger.info('Validation optimality gap: {:.3f}% +- {:.3f}'.format(mean_opt_gap, std_opt_gap))
    ...
    def train(self, args):
        ...
        model = _load_model(self.node_num + 1, prob_maps, weight_file=args.pretrained_weights, cuda_dev=cuda_dev)
        ...
        for epoch in range(num_epochs + 1):
            ...
            if epoch % 20 == 0:
                torch.save(model.state_dict(),
                           args.output / f"{int(time.time())}_intermediate_model{prob_maps}_{epoch}_{np.mean(epoch_losses):.2f}.torch")
...
def scp_test_validate(test_data,weight_file,problem):
    ...
    model = _load_model(node_num + 1, 32, weight_file=weight_file,
...
if __name__ == '__main__':
    # test
    test_data = "D:/dataset/20-mclp-test-feats/preprocessed/dgl_treesearch/graphs_weighted.dgl"
