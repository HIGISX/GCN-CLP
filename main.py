...
if __name__ == '__main__':
    ...
    parser.add_argument("--num_threads", type=int, nargs="?", action="store", default=1, help="Maximum number of threads to use.")
    parser.add_argument("--cuda_devices", type=int, nargs="*", action="store", default=[0], help="Which cuda devices should be used (distributed around the threads in round-robin fashion). If not given, CUDA is disabled.")
    ...
    parser.add_argument("--pretrained_weights", type=Path, nargs="?", default=None,action="store", help="Pre-trained weights to be used for solving/continuing training.")
    parser.add_argument("--lr", type=float, action="store", default=0.01, help="Learning rate (for training)")
    parser.add_argument("--epochs", type=int, action="store", default=20, help="Number of epochs to train for")
    parser.add_argument("--weighted", action="store_true", default=True, help="Whether input is weighted or unweighted.")
