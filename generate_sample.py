...
def generate_graph(input_file, output_path, problem):
    ...
    if weighted:
        weight_mapping = {vertex: int(weight) for vertex, weight in zip(G.nodes, costs)}
        nx.set_node_attributes(G, values=weight_mapping, name="weight")
    # add two feature:
    cover_num_mapping = {vertex: np.sum(cover) for vertex, cover in zip(G.nodes, C)}
    nx.set_node_attributes(G, values=cover_num_mapping, name="cover_num")
    cover_mapping = {vertex: covers for vertex, covers in zip(G.nodes, C)}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_path", type=str, action="store", help="Path of output files"
    )
    ...
    for file in file_list:
        ...
        if os.path.exists(output_file):
            continue
        label, cost_total = generate_graph(
            os.path.join(args.input_folder, file), args.output_path, args.problem
        )
