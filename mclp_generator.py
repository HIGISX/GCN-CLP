...
class MCLPGenerator():
    ...
    def __prepare_graph(nx_graph, weighted=False):
        ...
        if not weighted:
            g.ndata['weight'] = dgl.backend.tensor(np.ones(shape=(g.num_nodes(),1)), dtype=dgl.backend.data_type_dict['int8'])
        ...
        for graph_path in instance_directory.rglob("*.gpickle"):
            ...
            if source_mtime > last_updated or not os.path.exists(name_mapping_file):
                ...
                with open(range_mapping_file, "w", encoding='utf-8') as f:
                    json.dump(graph_covering_range, f, ensure_ascii=False, sort_keys = True, indent=4)
                break
